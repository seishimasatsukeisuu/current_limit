...
int main(void)
{
  ...
  MX_TIM1_Init();
  /* USER CODE BEGIN 2 */
  HAL_COMP_Start(&hcomp2); // COMP2を起動
  HAL_DAC_Start(&hdac1, DAC_CHANNEL_2);
  HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_2, DAC_ALIGN_12B_R, 3060);
  ...
  while (1)
  {
    ...
    while (1)
    {
      int comp = HAL_COMP_GetOutputLevel(&hcomp2);

      if (state == 0 && comp == COMP_OUTPUTLEVEL_HIGH)
      {
        state = 1;
        printf("HIGH\n");
      }
      else if (state == 1 && comp == COMP_OUTPUTLEVEL_LOW)
      {
        state = 0;
        printf("LOW\n");
      }
...
static void MX_COMP2_Init(void)
{
  ...
  hcomp2.Init.Output = COMP_OUTPUT_TIM1OCREFCLR;
